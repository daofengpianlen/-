...
function ensureMesTextHideStyle() {
  if ($(`head > style#${MES_TEXT_HIDE_CLASS}-style`).length) return;
  $(`<style id="${MES_TEXT_HIDE_CLASS}-style">.${MES_TEXT_HIDE_CLASS}{display:none!important;}</style>`).appendTo('head');
}

function hideMesText($mes_text: JQuery) {
  ensureMesTextHideStyle();
  $mes_text.addClass(MES_TEXT_HIDE_CLASS);
}
